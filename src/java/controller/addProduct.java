...
public class addProduct extends HttpServlet {
    ...
    /**
     * Handles the HTTP <code>GET</code> method. Forward to add-products.jsp
     * page
     */
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (copyFrom != null && !copyFrom.isEmpty()) {
                try {
                    int productId = Integer.parseInt(copyFrom);
                    ProductDAO dao = new ProductDAO();
                    // Lấy thông tin sản phẩm gốc
                    model.Products product = dao.findProductByID(productId);
                    ...
                    List<model.Categories> categories = dao.findAllCategoryOfProduct(productId);

                    // Tách loại sản phẩm và hãng
                    int categoryId = 0;
                    int brandId = 0;
                    for (model.Categories cate : categories) {
                        if (cate.getName().equalsIgnoreCase("Smartphones") || cate.getName().equalsIgnoreCase("Tablets")) {
                            categoryId = cate.getCategoryID();
                        } else {
                            brandId = cate.getCategoryID();
                        }
                    }

                    // Set sang request để JSP prefill
                    request.setAttribute("copyProduct", product);
                    request.setAttribute("copyProductDetails", productDetails);
                    request.setAttribute("copyGalleries", galleries);
                    request.setAttribute("copyCategoryId", categoryId);
                    request.setAttribute("copyBrandId", brandId);
                } catch (Exception ex) {
                    LOGGER.log(Level.WARNING, "Không thể lấy dữ liệu sản phẩm gốc để copy: {0}", ex.getMessage());
                }
            }
    ...
    /**
     * Handles the HTTP <code>POST</code> method. Process product insertion
     */
    @Override
    ...
    /**
     * Insert Product Details - Generate all color-storage combinations and
     * insert directly
     */
